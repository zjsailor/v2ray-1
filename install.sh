...
_load() {
	local _dir="/etc/v2ray/sinmists/v2ray/src/"
	. "${_dir}$@"
}
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		mkdir -p /etc/v2ray/sinmists/v2ray
		cp -rf $(pwd)/* /etc/v2ray/sinmists/v2ray
	else
		pushd /tmp
		if [[ $_test ]]; then
			git clone https://github.com/sinmists/v2ray -b test /etc/v2ray/sinmists/v2ray
		else
			git clone https://github.com/sinmists/v2ray /etc/v2ray/sinmists/v2ray
		fi
		popd

	fi

	if [[ ! -d /etc/v2ray/sinmists/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/sinmists/v2ray/config/backup.conf $backup
